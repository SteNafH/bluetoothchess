...
export default class MoveParser {
  public static getMoveFromAN(
    ...
    if (pieceName === 'O') {
      ...
      let previousPositions: Position[] = piece.getPreviousPositions(
        board,
      ...
      if (previousPositions.length === 0) {
        ...
        if (extraData.col !== undefined && extraData.row !== undefined) {
          previousPositions = previousPositions.filter(
            (position: Position) =>
              position.row === extraData.row && position.col === extraData.col,
          );
        } else if (extraData.col !== undefined) {
          previousPositions = previousPositions.filter(
            (position: Position) => position.col === extraData.col,
          );
        } else if (extraData.row !== undefined) {
          previousPositions = previousPositions.filter(
            (position: Position) => position.row === extraData.row,
          );
        }
    ...
    if (type === MoveTypes.Promote) {
      return new Move(
        ...
        newPosition,
        type,
        MoveParser.getPromotionPiece(move),
      );
    }
  ...
  public static getNewPosition(move: string): Position {
    ...
    //Ra4xb5 -> a4b5 -> b5
    const newPosition: string = move
      .replace(new RegExp(/[^a-h^1-8]+/g), '')
      .slice(-2);
