...
export default class Pawn extends Piece {
  ...
  public getPreviousPositions(board: ChessBoard, capture: boolean): Position[] {
    ...
    if (capture) {
      ...
      if (
        this.isPreviousPosition(
          board[this.position.row + rowDelta][this.position.col],
        )
      ) {
        possiblePositions.push(
          new Position(this.position.row + rowDelta, this.position.col),
        );
      }

      if (
        ((this.white && this.position.row === 3) ||
          (!this.white && this.position.row === 4)) &&
        board[this.position.row + rowDelta][this.position.col] === null
      ) {
        if (
          this.isPreviousPosition(
            board[this.position.row + rowDelta + rowDelta][this.position.col],
          )
        ) {
          possiblePositions.push(
            new Position(
              this.position.row + rowDelta + rowDelta,
              this.position.col,
            ),
          );
        }
